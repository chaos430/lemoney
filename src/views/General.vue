...
<a v-else @click="createBudget">￥{{$store.state.budget-result('-','month')}} </a>
  ...
  <div class="records">
    <router-link class="record" v-for="item in recordList" :key="item.createdAt" :to="`general/edit/${item.id}`">
      <Icon :name="item.tag"/>
      <span class="icon-span">{{item.tag}}<span>{{item.notes}}</span></span>
      <span class="amount"><span >￥{{item.type}}{{item.amount}}</span>{{beautify(item.createdAt)}}</span>
    </router-link>
  </div>
</Layout>
</template>
