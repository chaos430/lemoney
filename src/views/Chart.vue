<template>
  <Layout>
    ...
    <div class="summary">
      ...
      <div class="last"><span>共{{ this.billDetail('count', '+') }}笔收入，￥{{ this.billDetail('amount', '+') }}</span><span>结余￥{{ this.billDetail('amount', '+') - this.billDetail('amount', '-') }}</span>
      </div>
    </div>
    <Types class="type"/>
    <div>
    ...
    days(name, type) {
      ...
      for (let i = 1; i <= days; i++) {
        ...
        for (let j = 0; j < this.$store.state.recordList.length; j++) {
          if (dayjs(this.$store.state.recordList[j].createdAt).isSame(date, 'day')) {
